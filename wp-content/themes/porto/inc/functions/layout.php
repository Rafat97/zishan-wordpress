...
function porto_currency_switcher( $el_class = '' ) {
	...
	if ( ! $porto_settings['wcml-switcher-html'] && ! has_nav_menu( 'currency_switcher' ) && ( porto_is_elementor_preview() || porto_vc_is_inline() ) ) {
	...
	if ( $porto_settings['wcml-switcher'] && '' == $porto_settings['wcml-switcher-pos'] ) {
		if ( class_exists( 'WOOCS' ) ) {
			...
			foreach ( $currencies as $currency ) {
				if ( 1 == $woocommerce_wpml->settings['currency_options'][ $currency ]['languages'][ $current_language ] ) {
...
function porto_view_switcher( $el_class = '' ) {
	global $porto_settings;
	if ( ! $porto_settings['wpml-switcher-html'] && ! has_nav_menu( 'view_switcher' ) && ( porto_is_elementor_preview() || porto_vc_is_inline() ) ) {
...
function porto_mobile_menu( $secondary_menu = false ) {
	...
	$output = '';
	$first_index = 0;
	if ( ( true === $is_option && in_array( 'main', $porto_settings['show-mobile-menus'] ) ) || false == $is_option ) {
		if ( $menu_is_tab ) {
			$output .= '<div class="menu-wrap tab-pane' . ( 0 == $first_index ? ' active' : '' ) . '" tab-id="menu-main">';
			$first_index++;
		}
		ob_start();
	...
	if ( ( true === $is_option && in_array( 'secondary', $porto_settings['show-mobile-menus'] ) ) || ( false == $is_option && $secondary_menu ) ) {
		if ( $menu_is_tab ) {
			$output .= '<div class="menu-wrap tab-pane' . ( 0 == $first_index ? ' active' : '' ) . '" tab-id="menu-secondary">';
			$first_index++;
		}
		ob_start();
		$secondary_menu = porto_get_meta_value( 'secondary_menu' );
	...
	// sidebar menu
	if ( ( true === $is_option && in_array( 'side', $porto_settings['show-mobile-menus'] ) ) || false == $is_option ) {
		$sidebar_menu = porto_get_meta_value( 'sidebar_menu' );
		if ( $menu_is_tab ) {
			$output .= '<div class="menu-wrap tab-pane' . ( 0 == $first_index ? ' active' : '' ) . '" tab-id="menu-side">';
			$first_index++;
		}
		ob_start();
		if ( has_nav_menu( 'sidebar_menu' ) || $sidebar_menu ) {
...
function porto_search_form( $el_class = '' ) {
	...
	$result .= '<a '. apply_filters( 'porto_searchtoggle_data_attrs', '' ) . ' class="search-toggle" aria-label="Search Toggle" href="#"><i class="fas fa-search"></i><span class="search-text">' . esc_html__( 'Search', 'porto' ) . '</span></a>';
