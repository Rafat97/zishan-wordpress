...
jQuery( function( $ ) {
	...
	if ( $( '.blocks-wrapper .blocks-list' ).length ) {
		...
		$( 'body:not(.vcv-wb-editor)' ).on( 'click', '.blocks-list .import', function( e ) {
			...
			var $this = $( this ),
				...
				blockOptions = [ 108, 109, 110, 111, 112, 113, 114, 116, 117, 122, 123, 124, 125, 126, 127, 128, 129, 130, 134, 135, 138, 139, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 151, 153, 154, 157, 164, 166, 242, 243, 244, 245 ];
	...
	if ( location.hash ) {
		...
		if ( 'object' === typeof hrefTarget ) {
			...
			if ( typeof wp.customize == 'undefined' && $( '#redux_save' ).length ) {
				$optionTarget = $( '#porto_settings-' + hrefTarget );
				$tabTarget = $optionTarget.parents( '.redux-group-tab' ).data( 'rel' );
				$adminbarHeight = 0;
				if ( typeof $tabTarget != 'undefined' ) {
					// Check if target element exists.
					$theTarget = $( 'a[data-key="' + $tabTarget + '"]' );
					if ( $theTarget ) {
						setTimeout( function() {
							...
							setTimeout( function() {
								...
								$( 'html, body' ).animate( { scrollTop: $closeTr.offset().top - $adminbarHeight } );
				...
				if ( typeof $theTarget.contentContainer != 'undefined' ) {
					$.redux.initFields( $theTarget.contentContainer );
				}
				$theTarget.focus();

				if ( $particularOption.length ) {
					$adminbarHeight = $( '.wp-full-overlay-sidebar-content' ).offset().top;
					setTimeout( () => {
						$( '.wp-full-overlay-sidebar-content' ).animate( { scrollTop: $( $particularOption ).offset().top - $adminbarHeight } );
						...
						if ( $( '#porto_settings-' + name ).length ) {
							if ( value ) {
								...
								if ( 'menu-type' == name ) { // menu-type is ''
									$( '#porto_settings-' + name ).find( 'li:first-child input[value]' ).trigger( 'click' );
								} else {
